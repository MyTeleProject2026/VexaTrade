...
export default function AdminSpotTradePage(){
 ...
 const [enabled,setEnabled]=useState(true),[max,setMax]=useState("100000"),[loading,setLoading]=useState(true),[saving,setSaving]=useState(false);
 useEffect(()=>{let alive=true;adminApi.getSpotTradeSettings(token).then(r=>{if(!alive)return;const d=r.data?.data||{};setEnabled(d.trading_enabled!==false);setMax(String(d.max_order_usdt||100000));}).catch(e=>addToast(getApiErrorMessage(e),"error")).finally(()=>alive&&setLoading(false));return()=>{alive=false}},[token]);
 async function save(e){e.preventDefault();setSaving(true);try{const r=await adminApi.updateSpotTradeSettings({trading_enabled:enabled,max_order_usdt:Number(max)},token);if(!r.data?.success)throw new Error(r.data?.message||"Unable to save");addToast("Spot trading controls saved","success")}catch(e){addToast(getApiErrorMessage(e),"error")}finally{setSaving(false)}}
