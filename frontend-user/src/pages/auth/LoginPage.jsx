          ...
          <div className="relative z-10 flex w-full flex-col justify-between p-10 xl:p-14">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-sm text-cyan-300">
                <ShieldCheck size={16} /> VexaTrade Secure Access
              </div>
              <h1 className="mt-8 max-w-lg text-5xl font-bold leading-tight text-white xl:text-6xl">Welcome back to your trading world.</h1>
              <p className="mt-6 max-w-xl text-lg leading-8 text-slate-400">Sign in to access your assets, trading dashboard, wallet flow, profile center, and platform updates with a cleaner premium experience.</p>
            </div>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5"><div className="text-xs uppercase tracking-[0.28em] text-slate-500">Live</div><div className="mt-3 text-2xl font-semibold text-white">Market access</div></div>
              <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5"><div className="text-xs uppercase tracking-[0.28em] text-slate-500">Fast</div><div className="mt-3 text-2xl font-semibold text-white">Wallet actions</div></div>
              <div className="rounded-[28px] border border-white/10 bg-white/[0.03] p-5"><div className="text-xs uppercase tracking-[0.28em] text-slate-500">Smart</div><div className="mt-3 text-2xl font-semibold text-white">User center</div></div>
            </div>
          </div>
        </section>
          ...
          <div className="w-full max-w-md">
            <div className="rounded-[34px] border border-white/10 bg-[#0a0e1a] p-8 shadow-[0_25px_90px_rgba(0,0,0,0.5)]">
              <div className="mb-8 text-center">
                ...
                <p className="mt-3 text-sm text-slate-400">Sign in to access your trading dashboard.</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="Enter your email" className="w-full rounded-2xl border border-white/10 bg-[#0a0e1a] px-4 py-4 text-white outline-none focus:border-cyan-500" autoComplete="email" />
                </div>
                <div>
                  <label className="mb-2 block text-sm text-slate-400">Password</label>
                  <div className="relative">
                    <input type={showPassword ? "text" : "password"} name="password" value={form.password} onChange={handleChange} placeholder="Enter your password" className="w-full rounded-2xl border border-white/10 bg-[#0a0e1a] px-4 py-4 pr-12 text-white outline-none focus:border-cyan-500" autoComplete="current-password" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-white">{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button>
                  </div>
                </div>
              ...
              <div className="mt-6 space-y-2 text-center text-sm">
                ...
                <p className="text-slate-400"><Link to="/forgot-password" className="font-semibold text-slate-200 hover:text-white">Forgot password?</Link></p>
              </div>
              ...
              <div className="mt-8 rounded-[24px] border border-white/10 bg-white/[0.03] p-4">
                ...
                <div className="mt-3 text-sm leading-6 text-slate-300">Your session, wallet access, and user center actions are protected through platform authentication.</div>
              </div>
            </div>
          </div>
        </section>
