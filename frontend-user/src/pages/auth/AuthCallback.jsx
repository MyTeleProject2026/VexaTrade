...
const AuthCallback = () => {
  ...
  useEffect(() => {
    ...
    (async()=>{
      ...
      const expected=sessionStorage.getItem('vexa_sso_state'); const verifier=sessionStorage.getItem('vexa_sso_verifier');
      if(!code||!state||!expected||state!==expected||!verifier){navigate('/login?error=invalid_sso_state',{replace:true});return;}
      try{
        setMessage('Verifying your VexaAccount securely…');
        const response=await fetch(`${API_BASE_URL}/api/auth/vexaaccount/callback`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code,code_verifier:verifier})});
        ...
        sessionStorage.removeItem('vexa_sso_state');sessionStorage.removeItem('vexa_sso_verifier');
        window.history.replaceState({},document.title,window.location.pathname);navigate('/account-verification',{replace:true});
      }catch(err){console.error('VexaAccount callback:',err);sessionStorage.removeItem('vexa_sso_state');sessionStorage.removeItem('vexa_sso_verifier');navigate('/login?error=auth_failed',{replace:true});}
