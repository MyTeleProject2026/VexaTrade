...
export default function ShortTermMarketPage(){
 const [pair,setPair]=useState("BTCUSDT"); const [price,setPrice]=useState(0); const navigate=useNavigate();
 ...
 return <TradeSectionLayout title="Short-Term Market" subtitle="Live market view" mode="short"><div className="space-y-2.5">
  ...
  <div className="flex items-center justify-between rounded-xl border border-white/10 bg-[#0a0e1a] px-3 py-2"><span className="text-[9px] text-slate-500">Live price</span><b className="text-sm tabular-nums">{price ? price.toLocaleString(undefined,{maximumFractionDigits:8}) : "Connecting…"}</b></div><button onClick={()=>navigate("/trade/short-term/trade")} className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400 py-3 text-xs font-bold text-black"><Zap size={14}/> Trade {pair}</button>
