...
export default function UserCenterPage() {
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        if (data.hasJointAccount && data.jointAccount) {
          ...
          if (partnerUid) {
            try {
              const partnerRes = await fetch(`${import.meta.env.VITE_API_BASE_URL || "https://vexatrade-5ycu.onrender.com"}/api/user/by-uid/${partnerUid}`, {
  ...
  async function loadCombinedBalance() {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL || "https://vexatrade-5ycu.onrender.com"}/api/joint-account/combined-balance`, {
        headers: { Authorization: `Bearer ${token}` }
      });
