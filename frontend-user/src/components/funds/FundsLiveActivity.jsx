...
function money(v){return n(v).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:6})}

function LiveFund({fund,now}){
 ...
 const liveProfit=baseProfit+(principal*rate/100)*(elapsed/86400);
 const current=principal+liveProfit;
 const total=n(fund.total_days);
 const day=n(fund.current_day);
 ...
 return <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.045] p-3">
   ...
   <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
    ...
    <div><div className="text-[9px] text-slate-500">Live profit*</div><div className="text-xs font-semibold text-emerald-300">+{money(liveProfit)} USDT</div></div>
    <div><div className="text-[9px] text-slate-500">Live value*</div><div className="text-xs font-semibold text-cyan-300">{money(current)} USDT</div></div>
   ...
   <div className="mt-2 flex items-center justify-between text-[9px] text-slate-500"><span>Per-second display updates continuously</span><span>Server settlement remains authoritative</span></div>
 </div>
}
...
export default function FundsLiveActivity({compact=false,onCountChange}){
 ...
 const load=async(silent=true)=>{try{if(!silent)setRefreshing(true);const res=await fundsApi.active(token);const rows=Array.isArray(res?.data?.data)?res.data.data:[];setFunds(rows);setLastSync(Date.now());setError("");onCountChange?.(rows.length)}catch(e){setError("Live sync temporarily unavailable");}finally{setRefreshing(false)}};
 useEffect(()=>{load(false);const poll=setInterval(()=>load(true),3000);const clock=setInterval(()=>setNow(Date.now()),250);return()=>{clearInterval(poll);clearInterval(clock)}},[]);
 ...
 return <section className={compact?"rounded-xl border border-white/10 bg-[#0a0e1a] p-3":"rounded-2xl border border-white/10 bg-[linear-gradient(180deg,#0a0e1a,#050812)] p-3 shadow-lg"}>
   <div className="flex flex-wrap items-center justify-between gap-2">
    <div><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-cyan-300"><Activity size={12}/> Live Funds Processing</div><div className="mt-1 text-[11px] text-slate-400">{funds.length} active stream{funds.length===1?"":"s"} · {money(total)} USDT currently locked</div></div>
    <div className="flex items-center gap-2 text-[9px] text-slate-500"><ShieldCheck size={11} className="text-emerald-400"/> Secure server sync {lastSync?new Date(lastSync).toLocaleTimeString():"…"}</div>
   </div>
   <div className="mt-3 space-y-2">{funds.map(f=><LiveFund key={f.id} fund={f} now={now}/>)}</div>
   <div className="mt-3 flex flex-wrap gap-3 text-[9px] text-slate-500"><span className="inline-flex items-center gap-1"><Clock3 size={10}/> 3s server refresh</span><span className="inline-flex items-center gap-1"><CheckCircle2 size={10} className="text-emerald-400"/> Ledger values authoritative</span><span className="inline-flex items-center gap-1"><Wallet size={10}/> USDT settlement</span></div>
   {error&&<div className="mt-2 text-[9px] text-amber-300">{error}</div>}
   <div className="mt-2 text-[8px] text-slate-600">* Live profit/value is a client-side continuously updated projection between authoritative server settlement events; it does not create ledger credit by itself.</div>
 </section>
}
