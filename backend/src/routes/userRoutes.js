...
const { upload } = require('../../cloudinaryStorage'); // adjust path
...
router.post('/profile/upload-picture', authUser, upload.single('profile_picture'), async (req, res, next) => {
  try {
...
// ─── SECURITY ───────────────────────────────────────────────────────
router.post('/set-passcode', authUser, async (req, res, next) => {
  try {
...
router.get('/security-status', authUser, async (req, res, next) => {
  try {
    const [rows] = await pool.execute(
...
router.post('/send-email-verification-code', authUser, async (req, res, next) => {
...
router.post('/verify-email-code', authUser, async (req, res, next) => {
  const connection = await pool.getConnection();
  try {
...
router.post('/notifications/:id/read', authUser, async (req, res, next) => {
  try {
...
router.delete('/notifications/:id', authUser, async (req, res, next) => {
  try {
    const id = Number(req.params.id);
...
// ─── PASSCODE VERIFY ────────────────────────────────────────────────
router.post('/verify-passcode', authUser, async (req, res, next) => {
  try {
...
router.post('/kyc/upload', authUser, upload.fields([{ name: "front", maxCount: 1 }, { name: "back", maxCount: 1 }]), async (req, res, next) => {
  try {
    ...
    const frontUrl = frontFile.path; // cloudinary
