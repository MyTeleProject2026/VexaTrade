...
router.post('/convert/execute',authUser,async(req,res,next)=>{const c=await pool.getConnection();try{
 const fromCoin=String(req.body.fromCoin||'').trim().toUpperCase(),toCoin=String(req.body.toCoin||'').trim().toUpperCase(),fromAmount=Number(req.body.fromAmount||0);
 ...
 await c.beginTransaction();const [u]=await c.execute('SELECT id,status FROM users WHERE id=? FOR UPDATE',[req.user.id]);if(!u.length)throw createError(404,'User not found');if(['disabled','frozen'].includes(String(u[0].status||'').toLowerCase()))throw createError(403,'User not active');
 await debitAvailableAsset(c,{userId:req.user.id,coin:fromCoin,network:'INTERNAL',amount:fromAmount,referenceType:'convert',referenceId:null,note:`Convert ${fromCoin} to ${toCoin}`});
 await creditAssetBalance(c,{userId:req.user.id,coin:toCoin,network:'INTERNAL',amount:receive,referenceType:'convert',referenceId:null,note:`Converted ${fromCoin} to ${toCoin}`});
 const [result]=await c.execute(`INSERT INTO convert_transactions(user_id,from_coin,to_coin,from_amount,from_price_usdt,to_price_usdt,gross_usdt_value,fee_percent,fee_usdt,net_usdt_value,receive_amount,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,'completed',NOW(),NOW())`,[req.user.id,fromCoin,toCoin,fromAmount,fp,tp,gross,feePct,fee,net,receive]);
 await c.execute('UPDATE asset_ledger_entries SET reference_id=? WHERE user_id=? AND reference_type=? AND reference_id IS NULL',[result.insertId,req.user.id,'convert']);
 ...
 res.json({success:true,message:'Conversion completed',data:{id:result.insertId,fromCoin,toCoin,fromAmount,fromPriceUsdt:fp,toPriceUsdt:tp,grossUsdtValue:gross,feePercent:feePct,feeUsdt:fee,netUsdtValue:net,receiveAmount:receive}});
