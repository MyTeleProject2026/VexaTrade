...
router.get('/admin/control-center/health', authAdmin, async (req, res, next) => {
  ...
  try {
    const names = CHECKS.map(([, table]) => table);
    ...
    for (const [key, table] of CHECKS) {
      ...
      if (!available.has(table)) {
        checks[key] = { status: 'unavailable', table, durationMs: Date.now() - checkStarted, message: 'Database table is not present' };
        continue;
      }
      try {
        const [[row]] = await pool.execute(`SELECT COUNT(*) AS count FROM \`${table}\``);
        checks[key] = { status: 'ready', table, count: Number(row?.count || 0), durationMs: Date.now() - checkStarted };
      } catch (error) {
        checks[key] = { status: 'error', table, durationMs: Date.now() - checkStarted, message: error.message || 'Database check failed' };
      }
    }
