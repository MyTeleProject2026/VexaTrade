...
router.post('/withdrawals/request',authUser,async(req,res,next)=>{
 ...
 try{
  ...
  const idempotencyKey=String(req.get('Idempotency-Key')||req.body.idempotencyKey||'').trim().slice(0,128);\n  if(idempotencyKey){const [existing]=await connection.execute('SELECT id,status,coin,network,amount,fee_amount,net_amount FROM withdrawals WHERE user_id=? AND idempotency_key=? LIMIT 1 FOR UPDATE',[req.user.id,idempotencyKey]);if(existing.length){await connection.rollback();return res.json({success:true,message:'Existing withdrawal request returned',data:{...existing[0],replayed:true}});}}\n  const partner=await jointPartner(connection,req.user.id);
  const status=partner?'pending_joint_authorization':'pending_settlement';
  const [result]=await connection.execute(`INSERT INTO withdrawals (user_id,coin,network,address,amount,fee_amount,fee_type,net_amount,status,authorization_status,two_factor_verified_at,idempotency_key,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW(),NOW())`,[req.user.id,coin,network,address,amount,feeAmount,feeType,netAmount,status,partner?'joint_required':'authorized',twofa.required?new Date():null,idempotencyKey||null]);
