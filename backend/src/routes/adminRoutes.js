...
router.post('/admin/spot-settlement-rules', authAdmin, async (req, res, next) => {
  ...
  try {
    ...
    const [result] = await db.execute(
      "INSERT INTO spot_trade_settlement_profiles (name,status,settlement_model,price_source,min_order_usdt,max_order_usdt,max_slippage_bps,trading_fee_bps,quote_ttl_seconds,max_orders_per_day,pnl_enabled,pnl_reference,pnl_refresh_seconds,realized_pnl_on_sell,settlement_receipt_required,trading_enabled,buy_enabled,sell_enabled,supported_pairs,maintenance_message,created_by,updated_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      [name,status,'market_execution','binance_public_market',min,max,slippage,fee,ttl,daily,req.body?.pnl_enabled===false?0:1,'live_market',pnlRefresh,req.body?.realized_pnl_on_sell===false?0:1,1,tradingEnabled,buyEnabled,sellEnabled,supportedPairs.join(','),maintenanceMessage||null,req.admin.id,req.admin.id]
    );
...
router.put('/admin/spot-settlement-rules/:id', authAdmin, async (req,res,next)=>{
  ...
  try{
    ...
    const [result]=await db.execute("UPDATE spot_trade_settlement_profiles SET name=?,status=?,min_order_usdt=?,max_order_usdt=?,max_slippage_bps=?,trading_fee_bps=?,quote_ttl_seconds=?,max_orders_per_day=?,pnl_enabled=?,pnl_refresh_seconds=?,realized_pnl_on_sell=?,trading_enabled=?,buy_enabled=?,sell_enabled=?,supported_pairs=?,maintenance_message=?,updated_by=? WHERE id=?",[name,status,min,max,slippage,fee,ttl,daily,req.body?.pnl_enabled===false?0:1,pnlRefresh,req.body?.realized_pnl_on_sell===false?0:1,tradingEnabled,buyEnabled,sellEnabled,supportedPairs.join(','),maintenanceMessage||null,req.admin.id,id]);
