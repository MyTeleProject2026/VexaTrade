...
router.get('/start', (req, res) => {
  try {
    if (!JWT_SECRET || (isProduction && STATE_SECRET.length < 32)) return res.status(503).json({ success: false, message: 'VexaTrade SSO security is not configured' });
...
router.post('/callback', async (req, res) => {
  try {
    ...
    // VexaTrade's existing schema uses account_id as the central VexaAccount identity key.
    ...
    if (byAccount.length && byEmail.length && Number(byAccount[0].id) !== Number(byEmail[0].id)) {
      return res.status(409).json({ success: false, message: 'VexaAccount identity conflicts with an existing VexaTrade account' });
    }
    ...
    if (user) {
      ...
      const [fresh] = await pool.execute('SELECT * FROM users WHERE id=?', [user.id]);
      user = fresh[0];
    } else {
    ...
    // Do not expose the VexaAccount provider token as the VexaTrade API token.
    // Mint a VexaTrade-local session token that the existing auth middleware can verify.
