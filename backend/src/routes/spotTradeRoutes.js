...
router.post("/spot/orders",authUser,transactionSecurity("spot-trade"),async(req,res,next)=>{
 ...
 try{
  ...
  const quoteAmount=quantity*price,max=Number(cfg.max_order_usdt||100000),min=Number(cfg.min_order_usdt||10),slippageBps=Number(cfg.max_slippage_bps||100),feeBps=Math.max(0,Number(cfg.trading_fee_bps||0)),quoteTtl=Number(cfg.quote_ttl_seconds||15);
  ...
  await db.commit();res.json({success:true,message:"Spot order filled",data:{orderId:result.insertId,symbol,side,orderType:type,quantity,executionPrice:price,quoteAmount,feeAmount,feeBps,status:"filled",filledAt:new Date().toISOString()}});
