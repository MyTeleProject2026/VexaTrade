...
router.post("/spot/orders",authUser,transactionSecurity("spot-trade"),async(req,res,next)=>{
 ...
 try{
  ...
  const cfg=Object.fromEntries(settings.map(r=>[r.setting_key,r.setting_value])); if(cfg.trading_enabled==="false")throw createError(403,cfg.maintenance_message||"Spot trading is temporarily disabled");
